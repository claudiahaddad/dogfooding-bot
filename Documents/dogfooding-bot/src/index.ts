...
async function main() {
  ...
  const dogfoodingGroup = await findOrCreateDogfoodingGroup(client);
  ...
  log(`[INFO] Successfully found/created the TBA Dogfooding group: ${dogfoodingGroup.id}`);

  log("Syncing conversations...");
  await client.conversations.sync();

  log("Listening for messages...");
  await listenForMessages(client, dogfoodingGroup);
}
