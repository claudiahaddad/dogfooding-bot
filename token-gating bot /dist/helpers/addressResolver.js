...
const inboxToAddressCache = new Map();
/**
 ...
 */
export function extractAddressFromMessage(messageContent) {
 ...
 */
export async function canMessageAddress(client, address) {
    try {
 ...
 */
export async function resolveInboxIdToAddress(client, inboxId, messageContent) {
    // Check cache first
 ...
 */
export async function resolveMultipleInboxIdsToAddresses(client, inboxIds) {
    log(`[ADDRESS-RESOLVER] Resolving ${inboxIds.length} inbox IDs to addresses`);
    const results = await Promise.allSettled(inboxIds.map(async (inboxId) => ({
        inboxId,
