...
// --- End of Retry Logic ---
export async function listenForMessages(client, tokenGatedGroup) {
    let retryCount = 0;
        ...
        try {
            ...
            for await (const message of messageStream) {
                ...
                if (shouldSkip(message, client)) {
                    log(`[DEBUG] Skipping message ${message?.id}: Self-message or non-text content.`);
                    continue;
                }
                    ...
                    try {
                        ...
                        if (convIdForError) {
                            ...
                            if (errorConversation && !(errorConversation instanceof Group)) {
                                await errorConversation.send("Sorry, I encountered an error processing your message.");
                            }
                        }
                    }
                    catch (sendError) {
                        log(`Failed to send error message after processing error: ${sendError instanceof Error ? sendError.message : String(sendError)}`);
                    }
...
function shouldSkip(message, client) {
    if (!message) {
        return true;
    }
    return (isSameString(message.senderInboxId, client.inboxId) ||
        message.contentType?.typeId !== "text");
}
/**
 * Handle new conversations - automatically check token eligibility and add to group
 */
async function handleNewConversation(client, conversation, tokenGatedGroup, tokenConfig, tokenSymbol) {
    try {
