...
import { checkMultipleTokenBalances, getTokenSymbol, TokenGatingConfig } from "./helpers/tokenGating.js";
 ...
 */
export async function startMemberScanner(
  client: Client,
  group: Group<any>,
  tokenConfig: TokenGatingConfig
): Promise<void> {
...
async function scanGroupMembers(
  ...
  try {
    ...
    const membersWithAddresses = getCachedMemberAddresses(
      regularMembers.map(member => member.inboxId)
    );
    ...
    const balanceChecks = await checkMultipleTokenBalances(
      membersWithAddresses.map(member => member.address as `0x${string}`),
      tokenConfig
    );
 ...
 */
async function getAdminInboxIds(group: Group<any>): Promise<string[]> {
  try {
 ...
 */
async function notifyRemovedMember(
  client: Client,
  inboxId: string,
